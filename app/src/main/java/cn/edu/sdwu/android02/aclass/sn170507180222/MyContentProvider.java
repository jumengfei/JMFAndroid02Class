...
public class MyContentProvider extends ContentProvider {
    ...
    private static final String MIME_MAINTYPE_DIR="vnd.android.cursor.dir";//多条数据
    private static final String MIME_MAINTYPE_ITEM="vnd.android.cursor.item";//单条数据
    ...
    private static  UriMatcher uriMatcher;//声明为静态
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        //返回值代表删除的数目
        int count=0;
        switch (uriMatcher.match(uri)){//判断Uri为单条或者多条
    ...
    public String getType(Uri uri) {
        ...
        //返回共享数据的MIME类型
        //通过Uri数据，利用 UriMatcher 判断请求单条数据或者多条数据
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        switch (uriMatcher.match(uri)){
            case MULTIPLE_STUDENT:
                ...
                break;
            case SINGLE_STUDENT:
                String id=uri.getLastPathSegment();//得到id
                if (selection!=null&&selection.length()>0){
                    selection+=" and id="+id;
                }else {
                    selection="id="+id;
