...
public class ch15Activity1 extends AppCompatActivity {
    ...
    public void query(View view){
        String[] selColumns={"id","stuname","stuadd","studel"};
        Uri uri=Uri.parse(CONTENT_URI_STRING);
        Cursor cursor=contentResolver.query(uri,selColumns,null,null,null);
        while (cursor.moveToNext()){
            Log.i(ch15Activity1.class.toString(),cursor.getString(cursor.getColumnIndex("stuname"))+cursor.getString(cursor.getColumnIndex("stuname")));
        }
        cursor.close();
    }
    ...
    public void delete(View view){
        Uri uri=Uri.parse(CONTENT_URI_STRING+"/2");//删除ID为2的学生
